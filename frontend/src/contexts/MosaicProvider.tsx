...
export function MosaicProvider({ children }: { children: ReactNode }) {
  ...
  const webrtcConnectionManagerRef = useRef<WebRTCConnectionManager | null>(null);
  if (webrtcConnectionManagerRef.current === null) {
    webrtcConnectionManagerRef.current = new WebRTCConnectionManager(signalingServerRef.current);
  }
  ...
  return <MosaicContext.Provider value={contextValue}>{children}</MosaicContext.Provider>;
}
