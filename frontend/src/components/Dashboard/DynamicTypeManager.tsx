...
import { dynamicTypeManager, DynamicTypeConfig } from '../../dashboard/dynamic/dynamic-type-config'
...
export function DynamicTypeManager({ robotId, isOpen, onClose, onTypeUpdated }: DynamicTypeManagerProps) {
  const [dynamicTypes, setDynamicTypes] = useState<DynamicTypeConfig[]>([])
  const [isEditing, setIsEditing] = useState(false)
  const [editingConfig, setEditingConfig] = useState<DynamicTypeConfig | null>(null)
              ...
              {!isEditing ? (
                ...
                <VStack gap={6} align="stretch">
                  ...
                  <Box>
                    ...
                    <select
                      value={channelType}
                      onChange={(e) => setChannelType(e.target.value as 'readonly' | 'writeonly')}
                      style={{
                        width: '100%',
                        padding: '8px 12px',
  ...
  "required": ["temperature", "humidity"]
                    ...
                    />
                    {jsonSchema && (
                      <Text fontSize="xs" color={isValid ? 'green.600' : 'red.600'} mt={1}>
                        {isValid ? '✓ 유효한 JSON Schema입니다' : '✗ 유효하지 않은 JSON Schema입니다'}
                      </Text>
                    )}
                  </Box>
