...
export function useMosaicWebRTCConnection() {
  ...
  if (!context) {
    throw new Error("useMosaicWebRTCConnection must be used within a MosaicProvider");
  }
  ...
  const createConnection = async (robotIdList: string[]) => {
    ...
    for (const robotId of robotIdList) {
      ...
      if (channelRequirements.length === 0) {
        console.warn(`[${robotId}] Skip WebRTC connection: no channel requirements`);
        return;
      }

      // TODO: 하나 죽더라도 계속 해야함, Promise.allSettled() 조사 필요
      await webrtcConnectionManager.createConnection(robotInfo, channelRequirements);
    }
  };
