...
export class RobotInfo {
  ...
  public constructor(id: string, name: string, status: RobotStatus, robotConfigs: RobotConfig) {
    this._id = id;
    this._name = name;
    this._status = status;
  ...
  get needSync(): boolean {
    return this._status !== 0 && this._status !== 2 && this._status !== 4 && this._status !== 5;
  }
}
