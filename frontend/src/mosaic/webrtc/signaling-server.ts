import type { OnWsMessageType, SendWsMessageType } from "@/contexts/WebSocketContext.ts";
import type {
...
export class SignalingServer {
  ...
  public setRtcConnection(rtcConnection: WebRTCConnection): void {
    const existingConnection = this.rtcConnections.get(rtcConnection.rtcConnectionId);
  ...
  public sendSdpOffer(rtcConnectionId: string, offer: RTCSessionDescriptionInit): void {
    if (!offer.sdp) {
  ...
  public sendIceCandidate(rtcConnectionId: string, candidate: RTCIceCandidate): void {
    this.sendWsMessage({
  ...
  private async receiveIceCandidate(data: WsExchangeIceCandidateDto): Promise<void> {
    const { rtcConnectionId, iceCandidate } = data;
    const webRtcConnection = this.rtcConnections.get(rtcConnectionId);
    if (!webRtcConnection) {
      console.warn(`[${rtcConnectionId}] Received ICE candidate for unknown WebRTC connection`);
      return;
    }
  ...
  private async receiveSdpAnswer(data: WsSendSdpAnswerDto): Promise<void> {
    ...
    const webRtcConnection = this.rtcConnections.get(rtcConnectionId);
    if (!webRtcConnection) {
      console.warn(`[${rtcConnectionId}] Received SDP answer for unknown WebRTC connection`);
      return;
    }
  ...
  private registerSignalingWsMessageListener(): void {
    this.onWsMessage("signaling.send_sdp_answer", async (data: WsSendSdpAnswerDto) => {
