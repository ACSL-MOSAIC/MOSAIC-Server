...
import type {
  ...
  PPCData,
  PPCPoints,
} from "@/stores/ProgressivePointCloudStore/ppc.types.ts"
...
export class ProgressivePointCloudStore extends ReceivableStore<PPCData> {
  ...
  private processPPCChunk(ppcChunk: PPCChunk, ppcMeta: PPCMeta): PPCData {
    ...
    const dataView = new DataView(
      ppcChunk.data.buffer,
      ppcChunk.data.byteOffset,
    )
    ...
    for (let i = 0; i < numPoints; i++) {
      ...
      const x = dataView.getFloat32(
        pointOffset + ppcMeta.xOffset,
        !ppcMeta.isBigEndian,
      ) // little-endian
      const y = dataView.getFloat32(
        pointOffset + ppcMeta.yOffset,
        !ppcMeta.isBigEndian,
      )
      const z = dataView.getFloat32(
        pointOffset + ppcMeta.zOffset,
        !ppcMeta.isBigEndian,
      )
  ...
  private tryParseMeta(buffer: ArrayBuffer): PPCMeta | null {
    try {
      const protoPPCMeta = ProgressivePointCloud.PPCMeta.decode(
  ...
  private tryParseChunk(buffer: ArrayBuffer): PPCChunk | null {
    try {
      const protoPPCChunk = ProgressivePointCloud.PPCChunk.decode(
        new Uint8Array(buffer),
      )
      return {
