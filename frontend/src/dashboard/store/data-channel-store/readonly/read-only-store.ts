...
export abstract class ReadOnlyStore<T, I = string> extends DataStore<T, I> {
    ...
    public addDataChannel(channel: RTCDataChannel): void {
        this.dataChannels.set(channel.label, channel)
        console.log(`ReadOnlyStore[${this.robotId}] data channel added:`, channel.label)
    }

    // 채널 제거
    public removeDataChannel(channelLabel: string): void {
        this.dataChannels.delete(channelLabel)
        console.log(`ReadOnlyStore[${this.robotId}] data channel removed:`, channelLabel)
    }
    ...
    public getChannelInfo(): { connected: boolean; label?: string; state?: string } {
        const connectedChannels = Array.from(this.dataChannels.values())
            .filter(channel => channel.readyState === 'open')
        
        return {
            connected: connectedChannels.length > 0,
            label: connectedChannels.length > 0 ? connectedChannels[0].label : undefined,
            state: connectedChannels.length > 0 ? connectedChannels[0].readyState : 'closed'
        }
    }

    // 모든 채널 정보 가져오기
    public getAllChannelInfo(): Array<{ label: string; connected: boolean; state: string }> {
