...
public class MosaicRobotStatusHandler {
  ...
  public Mono<Void> handleWsMessage(WsMessage<?> wsMessage, RobotWsSession wsSession) {
    ...
    RobotUpdateStatusWsDto req =
        this.objectMapper.convertValue(wsMessage.getData(), RobotUpdateStatusWsDto.class);
  ...
  public Mono<Void> updateRobotStatus(RobotStatus robotStatus, RobotWsSession wsSession) {
    return this.updateRobotStatus(
        robotStatus, wsSession.getRobotPk(), wsSession.getOrganizationFk());
  }

  public Mono<Void> updateRobotStatus(RobotStatus robotStatus, UUID robotPk, UUID organizationFk) {
    return this.robotService
        .updateRobotStatus(robotStatus, robotPk, organizationFk)
        .then(this.notifyToSubscribers(robotStatus, robotPk, organizationFk));
  }

  private Mono<Void> notifyToSubscribers(
      RobotStatus robotStatus, UUID robotPk, UUID organizationFk) {
    WsMessage<UserUpdateRobotStatusWsDto> wsMessage =
        new WsMessage<>(TYPE_UPDATE, new UserUpdateRobotStatusWsDto(robotPk, robotStatus));
    this.wsSessionManager
        .getUserSessionsSubscribedToRobot(robotPk, organizationFk)
        .forEach(
            userWsSession -> {
              try {
                ...
                log.error(
                    "Failed to send robot status update to user: {}",
