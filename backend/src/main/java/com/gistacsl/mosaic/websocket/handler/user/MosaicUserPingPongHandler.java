...
public class MosaicUserPingPongHandler {
    ...
    public Mono<Void> handlePong(UUID pingId, UserWsSession session) {
        ...
        if (record != null && record.pingId.equals(pingId)) {
            log.debug("Pong received from user session: {} for pingId: {}", session.getSessionId(), pingId);
        } else {
            log.warn("Unexpected pong from user session: {} with pingId: {}", session.getSessionId(), pingId);
        }
    ...
    public void checkAndCloseTimeoutSessions() {
        ...
        this.pendingPings.values().forEach(record -> {
            ...
            if (secondsElapsed > TIMEOUT_SECONDS) {
                log.warn("User session timeout: {} (ping sent at: {}, elapsed: {}s)",
                        record.sessionId, record.sentAt, secondsElapsed);

                this.wsSessionManager.getUserSession(record.sessionId).ifPresent(session -> session.close()
                        .doOnSuccess(v -> {
                            log.info("User session closed due to ping timeout: {}", record.sessionId);
                            this.wsSessionManager.removeUserSession(record.sessionId);
                        })
                        .doOnError(e -> log.error("Failed to close user session: {}", record.sessionId, e))
                        .subscribe());
