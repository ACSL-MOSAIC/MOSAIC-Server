...
public class MosaicUserStatusHandler {
  ...
  private Mono<Void> handleSubscribe(WsMessage<?> wsMessage, UserWsSession wsSession) {
    UserSubscribeRobotStatusWsDto req =
        this.objectMapper.convertValue(wsMessage.getData(), UserSubscribeRobotStatusWsDto.class);
  ...
  private Mono<Void> handleUnsubscribe(WsMessage<?> wsMessage, UserWsSession wsSession) {
    ...
    log.debug(
        "User session {} unsubscribed from robots: {}", wsSession.getSessionId(), req.robotIds());
    return Mono.empty();
  }
}
