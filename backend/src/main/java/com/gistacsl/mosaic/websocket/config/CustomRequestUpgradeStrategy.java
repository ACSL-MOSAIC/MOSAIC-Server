...
public class CustomRequestUpgradeStrategy implements RequestUpgradeStrategy {
  ...
  public CustomRequestUpgradeStrategy(
      WsSessionManager wsSessionManager, WsMessageSender wsMessageSender) {
  ...
  public Mono<Void> upgrade(
    ...
    HttpServerExchange httpExchange =
        ServerHttpRequestDecorator.getNativeRequest(exchange.getRequest());
            ...
            Mono.deferContextual(
                (contextView) -> {
                  ...
                  if (httpExchange.getRequestURI().startsWith("/ws/robot")) {
                    callback =
                        new CustomRobotWSConnectionCallBack(
                            this.wsSessionManager,
                            this.wsMessageSender,
                            handshakeInfo,
                            ContextWebSocketHandler.decorate(webSocketHandler, contextView),
                            bufferFactory);
                  } else {
                    callback =
                        new CustomUserWSConnectionCallBack(
                            this.wsSessionManager,
                            this.wsMessageSender,
                            handshakeInfo,
                            ContextWebSocketHandler.decorate(webSocketHandler, contextView),
  ...
  private static class CustomRobotWSConnectionCallBack implements WebSocketConnectionCallback {
    ...
    CustomRobotWSConnectionCallBack(
    ...
    public void onConnect(
      ...
      RobotWsSession session =
          new RobotWsSession(sessionId, webSocketChannel, this.handshakeInfo, this.bufferFactory);
      ...
      this.handler
          .handle(session)
          .checkpoint(webSocketHttpExchange.getRequestURI() + " [UndertowRequestUpgradeStrategy]")
          .subscribe(session);
    }
  ...
  private static class CustomUserWSConnectionCallBack implements WebSocketConnectionCallback {
    ...
    CustomUserWSConnectionCallBack(
        WsSessionManager wsSessionManager,
        WsMessageSender wsMessageSender,
        HandshakeInfo handshakeInfo,
        WebSocketHandler handler,
    ...
    public void onConnect(
        WebSocketHttpExchange webSocketHttpExchange, WebSocketChannel webSocketChannel) {
      UUID sessionId = UUID.randomUUID();
      UserWsSession session =
          new UserWsSession(sessionId, webSocketChannel, this.handshakeInfo, this.bufferFactory);
