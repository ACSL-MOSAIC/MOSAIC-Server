...
public class WsMessageSender {
  ...
  public <R> void sendWsMessageToRobot(WsMessage<R> wsMessage, UUID targetSessionId)
      throws CustomException {
    Optional<RobotWsSession> optionalWsSession =
        this.wsSessionManager.getRobotSession(targetSessionId);
  ...
  public <R> void sendWsMessageToRobot(WsMessage<R> wsMessage, RobotWsSession wsSession)
      throws CustomException {
  ...
  public <R> void sendWsMessageToUser(WsMessage<R> wsMessage, UUID targetSessionId)
      throws CustomException {
    log.debug("Sending WebSocket response to User session: {}", targetSessionId);
    Optional<UserWsSession> optionalWsSession =
        this.wsSessionManager.getUserSession(targetSessionId);
    if (optionalWsSession.isEmpty()) {
      log.warn("WebSocket session not found: {}", targetSessionId);
      throw new CustomException(ResultCode.WEBSOCKET_SESSION_NOT_EXIST);
    }
  ...
  public <R> void sendWsMessageToUser(WsMessage<R> wsMessage, UserWsSession wsSession)
      throws CustomException {
    wsSession.sendMessage(this.wsMessageToString(wsMessage));
    log.debug("WebSocket message sent successfully to session: {}", wsSession.getSessionId());
  }
