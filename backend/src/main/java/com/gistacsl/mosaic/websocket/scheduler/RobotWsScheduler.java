        ...
        .subscribe(
            robot -> {
              ...
              this.wsSessionManager
                  .getAuthenticatedRobotSessionByRobotPk(robot.getPk())
                  .ifPresentOrElse(
                      ...
                      () -> {
                        if (robot.getStatus() != RobotStatus.DISCONNECTED) {
                          log.debug(
                              "Robot {} has no active session,"
                                  + " updating status to"
                                  + " DISCONNECTED",
                              robot.getPk());
                          this.mosaicRobotStatusHandler
                              .updateRobotStatus(
  ...
  public void checkUnauthenticatedSessions() {
    ...
    this.wsSessionManager
        ...
        .forEach(
            session -> {
              log.warn(
                  "Closing unauthenticated robot session: {} (connected at: {})",
              ...
              session
                  ...
                  .doOnSuccess(
                      v ->
                          log.info(
                              "Unauthenticated robot session closed:" + " {}",
                              session.getSessionId()))
                  .doOnError(
                      e ->
                          log.error(
                              "Failed to close unauthenticated robot" + " session: {}",
                              session.getSessionId(),
                              e))
                  .subscribe();
            });
