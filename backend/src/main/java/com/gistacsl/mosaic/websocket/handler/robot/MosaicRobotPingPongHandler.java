...
public class MosaicRobotPingPongHandler {
  ...
  public Mono<Void> handlePong(UUID pingId, RobotWsSession session) {
    ...
    if (record != null && record.pingId.equals(pingId)) {
      log.debug(
          "Pong received from robot session: {} for pingId: {}", session.getSessionId(), pingId);
    } else {
      log.warn(
          "Unexpected pong from robot session: {} with pingId: {}", session.getSessionId(), pingId);
    }
        ...
        .forEach(
            record -> {
              ...
              if (secondsElapsed > TIMEOUT_SECONDS) {
                log.warn(
                    "Robot session timeout: {} (ping sent at: {}, elapsed:" + " {}s)",
                    record.sessionId,
                    record.sentAt,
                    secondsElapsed);

                this.wsSessionManager
                    .getRobotSession(record.sessionId)
                    .ifPresent(
                        session ->
                            session
                                .close()
                                ...
                                .then(
                                    this.mosaicRobotStatusHandler.updateRobotStatus(
                                        RobotStatus.DISCONNECTED, session))
                                .doOnError(
                                    e ->
                                        log.error(
                                            "Failed to" + " close" + " robot" + " session:" + " {}",
                                            record.sessionId,
                                            e))
                                .subscribe());
