...
public class MosaicRobotAuthorizeHandler {
  ...
  public Mono<Void> handleWsMessage(WsMessage<?> wsMessage, RobotWsSession wsSession) {
    RobotAuthorizeWsDto<?> req =
        this.objectMapper.convertValue(wsMessage.getData(), RobotAuthorizeWsDto.class);

    return this.robotService
        .getRobotEntity(req.getRobotId())
        .flatMap(
            robotEntity -> {
              try {
                MosaicBaseAuthHandler authHandler =
                    this.authHandlerRegistry.getHandler(robotEntity.getAuthType());
                return authHandler
                    .handleMessage(req, robotEntity, wsSession)
                    .then(
                        Mono.defer(
                            () -> {
                              if (wsSession.getIsAuthenticated()) {
                                replyMessage(wsSession, ResultCode.SUCCESS);
                                return this.robotService.updateRobotStatus(
                                    RobotStatus.WS_CONNECTED,
                                    robotEntity.getPk(),
                                    robotEntity.getOrganizationFk());
                              } else {
