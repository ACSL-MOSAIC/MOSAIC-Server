...
public class MosaicUserSignalingHandler {
    ...
    try {
      switch (wsMessage.getType()) {
        case TYPE_SEND_SDP_OFFER -> {
          SendSdpOfferWsDto req =
              this.objectMapper.convertValue(wsMessage.getData(), SendSdpOfferWsDto.class);
          this.handleSendSdpOffer(req, wsSession);
        }
        case TYPE_EXCHANGE_ICE_CANDIDATE -> {
          ExchangeIceCandidateWsDto req =
              this.objectMapper.convertValue(wsMessage.getData(), ExchangeIceCandidateWsDto.class);
          this.handleExchangeIceCandidate(req, wsSession);
        }
        case TYPE_CLOSE_CONNECTION -> {
          CloseConnectionWsDto req =
              this.objectMapper.convertValue(wsMessage.getData(), CloseConnectionWsDto.class);
  ...
  private void handleSendSdpOffer(SendSdpOfferWsDto req, UserWsSession wsSession)
  ...
  private void handleExchangeIceCandidate(ExchangeIceCandidateWsDto req, UserWsSession wsSession)
      throws CustomException {
    WebRTCSession webRTCSession = this.webRTCSessionManager.getSession(req.rtcConnectionId());
    if (webRTCSession == null) {
      throw new CustomException(ResultCode.WEBRTC_SESSION_NOT_EXIST);
    ...
    WsMessage<ExchangeIceCandidateWsDto> wsMessage =
        new WsMessage<>(TYPE_EXCHANGE_ICE_CANDIDATE, req);
    this.wsMessageSender.sendWsMessageToRobot(wsMessage, webRTCSession.getRobotWsSession());
  }

  private void handleCloseConnection(CloseConnectionWsDto req, UserWsSession wsSession)
      throws CustomException {
    WebRTCSession webRTCSession = this.webRTCSessionManager.getSession(req.rtcConnectionId());
    if (webRTCSession == null) {
      throw new CustomException(ResultCode.WEBRTC_SESSION_NOT_EXIST);
