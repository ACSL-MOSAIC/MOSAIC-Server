...
public class UserService {
  ...
  public Mono<UserDto.Res> getCurrentUser(UserAuth userAuth) {
    return userRepository
        .findByPk(userAuth.getUserPk(), dslContext)
        ...
        .flatMap(
            user ->
                organizationRepository
                    ...
                    .switchIfEmpty(
                        Mono.error(new CustomException(ResultCode.ORGANIZATION_NOT_FOUND)))
  ...
  public Mono<MessageDto> updateCurrentUser(UserAuth userAuth, UserUpdateMeDto.Req req) {
    return userRepository
        ...
        .switchIfEmpty(Mono.error(new CustomException(ResultCode.USER_NOT_FOUND)))
        .flatMap(
            user -> userRepository.updateFullName(userAuth.getUserPk(), req.fullName(), dslContext))
        ...
        .flatMap(
            user -> {
              ...
              return userRepository.updatePassword(
                  userAuth.getUserPk(), newHashedPassword, dslContext);
            })
