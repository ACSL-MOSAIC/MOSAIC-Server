...
public class WebRTCService {
    ...
    public Mono<WebRTCConnectionDto.Res> createConnection(UserAuth userAuth, WebRTCConnectionDto.Req req) {
        log.info("Creating WebRTC connection for user {} with robots {}",
                userAuth.getUserPk(), req.robotIds());

        // Fetch all robots from DB in a single query
        return robotRepository.findByPkIn(req.robotIds(), dslContext)
                .collectList()
                .flatMap(robots -> validateRobots(robots, req.robotIds(), userAuth))
                .flatMap(robots -> createWebRTCSession(robots, userAuth));
    }

    private Mono<List<RobotEntity>> validateRobots(List<RobotEntity> robots, List<UUID> requestedRobotIds, UserAuth userAuth) {
        ...
        boolean allSameOrganization = robots.stream()
        ...
        boolean allReady = robots.stream()
                .allMatch(robot -> robot.getStatus() == RobotStatus.READY_TO_CONNECT);
    ...
    private Mono<WebRTCConnectionDto.Res> createWebRTCSession(List<RobotEntity> robots, UserAuth userAuth) {
        ...
        return Mono.justOrEmpty(wsSessionManager.getUserSessionByUserPk(userAuth.getUserPk()))
                ...
                .flatMap(userWsSession -> Flux.fromIterable(robots)
                        .flatMap(robot -> Mono.fromCallable(() -> {
                            WebRTCSession session = webRTCSessionManager.createNewSession(
                                    userWsSession.getSessionId(),
                                    robot.getPk()
                            );

                            log.info("Created WebRTC session {} for user {} and robot {}",
                                    session.getSessionId(), userAuth.getUserPk(), robot.getPk());
