...
public class RobotService {
            ...
            dslContext.transactionPublisher(
                configuration -> {
                  ...
                  String connectorConfig =
                      req.connectorConfig() != null ? req.connectorConfig() : "{\"connectors\":[]}";

                  RobotEntity newRobot =
                      RobotEntity.builder()
                          .pk(robotPk)
  ...
  public Mono<RobotInfoDto.Res> getRobot(UserAuth userAuth, UUID robotPk) {
    return robotRepository
        .findByPkAndOrganizationFk(robotPk, userAuth.getOrganizationPk(), dslContext)
    ...
    return Mono.from(
            dslContext.transactionPublisher(
                configuration -> {
                  ...
                  return robotRepository
                      .findByPkAndOrganizationFk(robotPk, userAuth.getOrganizationPk(), txContext)
                      .switchIfEmpty(Mono.error(new CustomException(ResultCode.ROBOT_NOT_FOUND)))
                      .flatMap(
                          existingRobot ->
                              robotRepository.updateRobot(
                                  robotPk,
            ...
            dslContext.transactionPublisher(
                configuration -> {
                  ...
                  return robotRepository
                      ...
                      .flatMap(
                          existingRobot ->
                              robotRepository.deleteByPkAndOrganizationFk(
                                  robotPk, userAuth.getOrganizationPk(), txContext));
                }))
    ...
    return Mono.from(
            dslContext.transactionPublisher(
                configuration -> {
                  ...
                  return robotRepository
                      .updateRobotStatus(status, robotPk, organizationPk, txContext)
                      .switchIfEmpty(Mono.error(new CustomException(ResultCode.ROBOT_NOT_FOUND)));
                }))
        .then();
  }
