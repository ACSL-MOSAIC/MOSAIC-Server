...
public class OccupancyMapService {
  ...
  public Mono<OccupancyMapListDto.Res> listOccupancyMaps(UserAuth userAuth, int skip, int limit) {
    return occupancyMapRepository
        .countByOrganizationFk(userAuth.getOrganizationPk(), dslContext)
        .flatMap(
            count ->
                occupancyMapRepository
                    .findAllByOrganizationFk(userAuth.getOrganizationPk(), skip, limit, dslContext)
  ...
  public Mono<MessageDto> createOccupancyMap(
      UserAuth userAuth, String name, Mono<FilePart> pgmFile, Mono<FilePart> yamlFile) {
    return Mono.from(
            ...
            configuration -> {
              ...
              return saveFile(pgmFile, pgmPath)
                  ...
                  .then(
                      Mono.defer(
                          () -> {
                            OccupancyMapEntity entity =
                                OccupancyMapEntity.builder()
                                    .pk(mapId)
                            ...
                            return occupancyMapRepository
                                .insertOccupancyMap(entity, txContext)
                                .map(
                                    pk ->
                                        new MessageDto(
                                            "Occupancy" + " map added" + " successfully"));
                          }));
            }));
  }
  ...
  public Mono<OccupancyMapDto.Res> getOccupancyMap(UserAuth userAuth, UUID id) {
    return occupancyMapRepository
        .findByPkAndOrganizationFk(id, userAuth.getOrganizationPk(), dslContext)
    ...
    return Mono.from(
        dslContext.transactionPublisher(
            configuration -> {
              ...
              return occupancyMapRepository
                  .findByPkAndOrganizationFk(id, userAuth.getOrganizationPk(), txContext)
                  .switchIfEmpty(
                      Mono.error(new CustomException(ResultCode.OCCUPANCY_MAP_NOT_FOUND)))
                  .flatMap(
                      entity -> {
                        // Delete files
                        return deleteFileIfExists(entity.getPgmFilePath())
                            ...
                            .then(
                                occupancyMapRepository.deleteByPkAndOrganizationFk(
  ...
  public Mono<byte[]> downloadMapZip(UserAuth userAuth, UUID id) {
    return occupancyMapRepository
        ...
        .flatMap(
            entity ->
                Mono.zip(readFile(entity.getPgmFilePath()), readFile(entity.getYamlFilePath()))
                    .flatMap(
                        tuple -> {
