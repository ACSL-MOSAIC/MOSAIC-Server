...
public class DashboardService {
  ...
  public Mono<List<TabInfoDto.Res>> listTabs(UserAuth userAuth) {
    return tabRepository
        .findAllByOrganizationFkWithoutConfig(userAuth.getOrganizationPk(), dslContext)
        .map(tabEntity -> new TabInfoDto.Res(tabEntity.getPk(), tabEntity.getName()))
        .collectList();
  }
  ...
  public Mono<TabConfigDto.Res> getTabConfig(UserAuth userAuth, UUID tabPk) {
    return Mono.from(
            tabRepository.findByPkAndOrganizationFk(
                tabPk, userAuth.getOrganizationPk(), dslContext))
        .map(
            tabEntity ->
                new TabConfigDto.Res(
                    tabEntity.getPk(), tabEntity.getName(), tabEntity.getTabConfig()));
  }
            ...
            dslContext.transactionPublisher(
                configuration -> {
                  ...
                  return tabRepository.updateTabName(
                      tabPk, userAuth.getOrganizationPk(), req.name(), txContext);
                }))
        .map(count -> new MessageDto("Tab name updated successfully"));
  }

  public Mono<MessageDto> updateTabConfig(
      UserAuth userAuth, UUID tabPk, TabConfigUpdateDto.Req req) {
    return Mono.from(
            dslContext.transactionPublisher(
                configuration -> {
                  DSLContext txContext = configuration.dsl();
                  ...
                  return tabRepository.updateTabConfig(
                      tabPk, userAuth.getOrganizationPk(), req.tabConfig(), txContext);
                }))
        .map(count -> new MessageDto("Tab name updated successfully"));
  }
            ...
            dslContext.transactionPublisher(
                configuration -> {
                  ...
                  return tabRepository.deleteByPkAndOrganizationFk(
                      tabPk, userAuth.getOrganizationPk(), txContext);
                }))
