...
public class OrganizationRepository {
  ...
  public Mono<OrganizationEntity> findByPk(UUID pk, DSLContext dslContext) {
    return Mono.from(dslContext.selectFrom(ORGANIZATION).where(ORGANIZATION.PK.eq(pk)))
  ...
  public Mono<OrganizationEntity> findByName(String name, DSLContext dslContext) {
    return Mono.from(dslContext.selectFrom(ORGANIZATION).where(ORGANIZATION.NAME.eq(name)))
        .onErrorMap(e -> new CustomException(ResultCode.DB_ORGANIZATION_READ_FAILED, e))
        .map(
            record ->
                OrganizationEntity.builder()
