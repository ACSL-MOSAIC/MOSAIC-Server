...
public class UserRepository {
  ...
  public Mono<UserEntity> findByEmail(String email, DSLContext dslContext) {
    return Mono.from(dslContext.selectFrom(USERS).where(USERS.EMAIL.eq(email)))
  ...
  public Mono<UserEntity> findByPk(java.util.UUID pk, DSLContext dslContext) {
    return Mono.from(dslContext.selectFrom(USERS).where(USERS.PK.eq(pk)))
        .onErrorMap(e -> new CustomException(ResultCode.DB_USER_READ_FAILED, e))
        .map(
            record ->
                UserEntity.builder()
  ...
  public Mono<Integer> updatePassword(
      java.util.UUID pk, String hashedPassword, DSLContext dslContext) {
    return Mono.from(
            dslContext
                .update(USERS)
  ...
  public Mono<Integer> countByOrganizationFk(UUID organizationFk, DSLContext dslContext) {
    return Mono.from(
            dslContext.selectCount().from(USERS).where(USERS.ORGANIZATION_FK.eq(organizationFk)))
        .onErrorMap(e -> new CustomException(ResultCode.DB_USER_READ_FAILED, e))
        .map(record -> record.value1());
  }

  public Flux<UserEntity> findAllByOrganizationFk(
      UUID organizationFk, int skip, int limit, DSLContext dslContext) {
    return Flux.from(
            dslContext
                .selectFrom(USERS)
  ...
  public Mono<Integer> updateUser(
    ...
    var query = dslContext.update(USERS).set(USERS.UPDATED_AT, java.time.OffsetDateTime.now());
    ...
    return Mono.from(query.where(USERS.PK.eq(pk).and(USERS.ORGANIZATION_FK.eq(organizationFk))))
        .onErrorMap(e -> new CustomException(ResultCode.DB_USER_UPDATE_FAILED, e));
  }

  public Mono<Integer> deleteByPk(UUID pk, DSLContext dslContext) {
    return Mono.from(dslContext.deleteFrom(USERS).where(USERS.PK.eq(pk)))
