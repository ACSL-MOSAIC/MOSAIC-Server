...
public class TabRepository {
  ...
  public Flux<TabEntity> findAllByOrganizationFkWithoutConfig(UUID organizationFk, DSLContext dsl) {
    return Flux.from(
            dsl.select(TAB.PK, TAB.ORGANIZATION_FK, TAB.NAME, TAB.CREATED_AT, TAB.UPDATED_AT)
                .from(TAB)
  ...
  public Mono<UUID> updateTabConfig(
      UUID pk, UUID organizationFk, String tabConfig, DSLContext dsl) {
    return Mono.from(
            dsl.update(TAB)
  ...
  public Mono<Integer> deleteByPkAndOrganizationFk(UUID pk, UUID organizationFk, DSLContext dsl) {
    return Mono.from(
            dsl.deleteFrom(TAB).where(TAB.PK.eq(pk)).and(TAB.ORGANIZATION_FK.eq(organizationFk)))
