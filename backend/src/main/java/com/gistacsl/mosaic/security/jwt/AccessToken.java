...
public class AccessToken extends JsonWebToken {
  ...
  public String issueToken(JwtPayload jwtPayload) throws CustomException {
    ...
    try {
      return super.doGenerateToken(
          claims,
          access_token_validation_time,
          mosaicKeyService.getPrivateKey(MosaicKeyService.PURPOSE_JWT));
    } catch (Exception e) {
  ...
  public Boolean verifyAccessTokenPayload(Claims tokenClaims, JwtPayload jwtPayload) {
    ...
    return userPk.equals(jwtPayload.userPk())
        && organizationPk.equals(jwtPayload.organizationPk())
        && role != null;
  }
