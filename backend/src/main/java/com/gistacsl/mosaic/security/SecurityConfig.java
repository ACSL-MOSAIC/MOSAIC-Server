        ...
        .authorizeExchange(
            exchanges ->
                exchanges
                    ...
                    .permitAll()
                    .pathMatchers("/api/v1/account/login/access-token")
                    .permitAll()
                    .pathMatchers("/api/v1/account/login/organization/access-token")
                    .permitAll()
                    .pathMatchers("/api/v1/account/signup")
                    .permitAll()
                    .pathMatchers("/api/v1/organization/**")
                    .hasRole("ORGANIZATION_ADMIN")
                    .pathMatchers("/api/**")
                    .authenticated())
        .addFilterAt(
            new BearerTokenAuthenticationFilter(jwtTokenService),
            SecurityWebFiltersOrder.AUTHENTICATION)
        .exceptionHandling(
            exceptionHandlingSpec -> {
              exceptionHandlingSpec.accessDeniedHandler(
                  (exchange, e) ->
                      Mono.error(new CustomRuntimeException(ResultCode.ACCESS_DENIED, e)));
              exceptionHandlingSpec.authenticationEntryPoint(
                  (exchange, e) ->
                      Mono.error(new CustomRuntimeException(ResultCode.AUTHENTICATION_FAILED, e)));
            })
