...
public class MosaicKeyService {
  ...
  private static final List<String> KEY_PURPOSES =
      Arrays.asList(PURPOSE_JWT, PURPOSE_ROBOT_SIMPLE_TOKEN_AUTH);
  ...
  // RSA
  public static PrivateKey stringToPrivateKey(String key, String algorithm)
  ...
  // RSA
  public static PublicKey stringToPublicKey(String key, String algorithm)
      throws UnsupportedEncodingException, GeneralSecurityException {
  ...
  private KeyPair generateKeyPair(int keySize, String algorithm)
  ...
  private Mono<KeyPair> loadOrGenerateKeyPair(String purpose) {
    return keyPairRepository
        .findByPurpose(purpose, dslContext)
        .flatMap(
            entity -> {
              try {
        ...
        .switchIfEmpty(
            Mono.defer(
                () -> {
                  log.info(
                      "Key pair not found for purpose: {}. Generating new" + " one...", purpose);
                  try {
  ...
  private KeyPair loadKeyPairFromEntity(KeyPairEntity entity) throws Exception {
    ...
    String decryptedPrivateKey =
        keyEncryptionService.decryptPrivateKey(entity.getEncryptedPrivateKey());
