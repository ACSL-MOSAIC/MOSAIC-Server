...
from app.schemas.dynamic_type_config import DynamicTypeConfig
...
class DynamicTypeConfigRepository:
    ...
    def get_by_user(self, user_id: UUID) -> DynamicTypeConfig | None:
        """사용자별 동적 타입 설정 조회"""
        statement = select(DynamicTypeConfig).where(
            DynamicTypeConfig.user_id == user_id
        )
        return self.session.exec(statement).first()

    def upsert(
        self, user_id: UUID, configuration: list[dict[str, Any]]
    ) -> DynamicTypeConfig:
        ...
        if existing_config:
            ...
            # 새 설정 생성
            new_config = DynamicTypeConfig(user_id=user_id, configuration=configuration)
