...
from app.schemas import DynamicTypeConfigCreate, DynamicTypeConfigPublic, Message

router = APIRouter(tags=["dynamic-type-config"])


@router.get("/", response_model=DynamicTypeConfigPublic)
def read_dynamic_type_config(
    ...
    if not config:
        ...
        return DynamicTypeConfigPublic(
            id=0,
...
def upsert_dynamic_type_config(
    *, session: SessionDep, current_user: CurrentUser, config_in: DynamicTypeConfigCreate
) -> Any:
    """
    ...
    config = config_repo.upsert(
        user_id=current_user.id,
        configuration=config_in.configuration
    )
    return config


@router.delete("/", response_model=Message)
def delete_dynamic_type_config(
    session: SessionDep, current_user: CurrentUser
) -> Any:
    """
    현재 사용자의 동적 타입 설정 삭제
