...
class ConnectionManager:
    ...
    async def connect_robot(self, websocket: WebSocket, robot_id: str) -> None:
        await websocket.accept()
    ...
    async def send_to_robot(self, message: dict, robot_id: str) -> None:
        if robot_id in self.active_connections:
            await self.active_connections[robot_id].send_json(message)

    def get_robot_connection(self, robot_id: str) -> WebSocket | None:
